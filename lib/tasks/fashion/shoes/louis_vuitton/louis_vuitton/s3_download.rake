...
namespace :shoes_lv_lv do

  desc "Download Louis Vuitton shoes data to AWS S3"
	task s3_download: :environment do

		main_bucket = ENV['AWS_BUCKET']
		main_folder = 'shoes/louisvuitton/'
		...
		items = ImageItem.where("source = ? AND is_downloaded = ? AND item_type = ? AND manufacturer = ?", "louis_vuitton", false, "shoes", "Louis Vuitton")
