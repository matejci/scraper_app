...
namespace :cloth_rc_netaporter do

   desc "Download Roberto Cavalli cloth data to AWS S3"
	task s3_download: :environment do

		main_bucket = ENV['AWS_TEST_BUCKET']
		main_folder = 'cloth/robertocavalli/'
		...
		items = ImageItem.where("source = ? AND is_downloaded = ? AND item_type = ? AND manufacturer = ?", "net-a-porter", false, "cloth", "Roberto Cavalli")
