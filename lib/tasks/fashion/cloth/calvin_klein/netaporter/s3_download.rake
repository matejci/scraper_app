...
namespace :cloth_ck_netaporter do

   desc "Download Calvin Klein cloth data to AWS S3"
	task s3_download: :environment do

		main_bucket = ENV['AWS_TEST_BUCKET']
		main_folder = 'cloth/calvinklein/'
		...
		items = ImageItem.where("source = ? AND is_downloaded = ? AND item_type = ? AND manufacturer = ?", "net-a-porter", false, "cloth", "Calvin Klein")
