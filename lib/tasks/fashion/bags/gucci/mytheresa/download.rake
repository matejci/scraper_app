...
namespace :bags_gucci_mytheresa_old do

	desc "Download Gucci bags data on local disk"
	task download: :environment do

		main_path = "#{Rails.root}/downloaded_data/fashion/bags/gucci/"

		items = Item.where("source = ? AND is_downloaded = ? AND item_type = ? AND manufacturer = ?", "mytheresa", false, "bags", "Gucci")
	...
	task s3_download: :environment do
		...
		main_bucket = 'fashion-bags-gucci'
