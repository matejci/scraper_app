...
namespace :bags_dior_dior do
  ...
  desc "Download Dior bags data to AWS S3"
	task s3_download: :environment do

		main_bucket = ENV['AWS_BUCKET']
		main_folder = 'bags/dior/'
		...
		items = ImageItem.where("source = ? AND is_downloaded = ? AND item_type = ? AND manufacturer = ?", "dior", false, "bags", "Dior")
