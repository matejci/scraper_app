...
namespace :bags_dg_netaporter_old do
	...
	task download: :environment do
		...
		items = Item.where("source = ? AND is_downloaded = ? AND item_type = ? AND manufacturer = ?", "net-a-porter", false, "bags", "DolceGabbana")
	...
	desc "Download DolceGabbana bags data to AWS S3"
	task s3_download: :environment do

		main_bucket = 'fashion-bags-dolcegabbana'
