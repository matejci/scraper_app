...
namespace :cloth_gucci_netaporter_old do

	desc "Download Gucci cloth data on local disk"
	task download: :environment do

		main_path = "#{Rails.root}/downloaded_data/fashion/cloth/gucci/"

		items = Item.where("source = ? AND is_downloaded = ? AND item_type = ? AND manufacturer = ?", "net-a-porter", false, "cloth", "Gucci")
	...
	desc "Download Gucci cloth data to AWS S3"
	task s3_download: :environment do

		main_bucket = 'fashion-cloth-gucci'
