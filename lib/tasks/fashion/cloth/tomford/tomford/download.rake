...
namespace :cloth_tomford_tomford_old do

	desc "Download Tom Ford cloth data on local disk"
	task download: :environment do

		main_path = "#{Rails.root}/downloaded_data/fashion/cloth/tomford/"

		items = Item.where("source = ? AND is_downloaded = ? AND item_type = ? AND manufacturer = ?", "tomford", false, "cloth", "TomFord")
	...
	desc "Download Tom Ford cloth data to AWS S3"
