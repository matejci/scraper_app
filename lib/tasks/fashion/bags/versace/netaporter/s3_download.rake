...
namespace :bags_versace_netaporter do

   desc "Download Versace bags data to AWS S3"
	task s3_download: :environment do

		main_bucket = ENV['AWS_TEST_BUCKET']
		main_folder = 'bags/versace/'
		...
		items = ImageItem.where("source = ? AND is_downloaded = ? AND item_type = ? AND manufacturer = ?", "net-a-porter", false, "bags", "Versace")
