...
namespace :shoes_dior_dior do

  desc "Download Dior shoes data to AWS S3"
	task s3_download: :environment do

		main_bucket = ENV['AWS_TEST_BUCKET']
		main_folder = 'shoes/dior/'
		...
		items = ImageItem.where("source = ? AND is_downloaded = ? AND item_type = ? AND manufacturer = ?", "dior", false, "shoes", "Dior")
