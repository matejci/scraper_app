...
namespace :bags_gr_yoox do

	desc "Download Gianvito Rossi bags data to AWS S3"
	task s3_download: :environment do

		main_bucket = ENV['AWS_TEST_BUCKET']
		main_folder = 'bags/gianvitorossi/'
		...
		items = ImageItem.where("source = ? AND is_downloaded = ? AND item_type = ? AND manufacturer = ?", "yoox", false, "bags", "Gianvito Rossi")
