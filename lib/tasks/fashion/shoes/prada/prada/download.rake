...
namespace :shoes_prada_prada_old do

	desc "Download Prada shoes data on local disk"
	task download: :environment do

		main_path = "#{Rails.root}/downloaded_data/fashion/shoes/prada/"

		items = Item.where("source = ? AND is_downloaded = ? AND item_type = ? AND manufacturer = ?", "prada", false, "shoes", "Prada")
	...
	desc "Download Prada shoes data to AWS S3"
	task s3_download: :environment do

		main_bucket = 'fashion-shoes-prada'
		...
		items.each do |item|
			...
			urls_arr.each do |url|
				begin
					stream = open(url)
					file_name = file_name = url.split("/")[8]
