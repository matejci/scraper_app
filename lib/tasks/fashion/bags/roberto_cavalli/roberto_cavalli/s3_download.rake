...
namespace :bags_rc_rc do

  desc "Download Roberto Cavalli bags data to AWS S3"
	task s3_download: :environment do

		main_bucket = ENV['AWS_TEST_BUCKET']
		main_folder = 'bags/robertocavalli/'
		...
		items = ImageItem.where("source = ? AND is_downloaded = ? AND item_type = ? AND manufacturer = ?", "roberto_cavalli", false, "bags", "Roberto Cavalli")
