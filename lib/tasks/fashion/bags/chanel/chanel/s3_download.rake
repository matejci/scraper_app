...
namespace :bags_chanel_chanel do
  ...
  desc "Download Chanel bags data to AWS S3"
	task s3_download: :environment do

		main_bucket = ENV['AWS_BUCKET']
		main_folder = 'bags/chanel/'
		...
		items = ImageItem.where("source = ? AND is_downloaded = ? AND item_type = ? AND manufacturer = ?", "chanel", false, "bags", "Chanel")
