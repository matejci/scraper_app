...
namespace :shoes_dg_netaporter_old do

	desc "Download DolceGabbana shoes data on local disk"
	task download: :environment do

		main_path = "#{Rails.root}/downloaded_data/fashion/shoes/dolcegabbana/"

		items = Item.where("source = ? AND is_downloaded = ? AND item_type = ? AND manufacturer = ?", "net-a-porter", false, "shoes", "DolceGabbana")
	...
	desc "Download DolceGabbana shoes data to AWS S3"
	task s3_download: :environment do

		main_bucket = 'fashion-shoes-dolcegabbana'
