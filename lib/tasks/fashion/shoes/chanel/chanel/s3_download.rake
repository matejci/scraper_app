...
namespace :shoes_chanel_chanel do

  desc "Download Chanel shoes data to AWS S3"
	task s3_download: :environment do

		main_bucket = ENV['AWS_BUCKET']
		main_folder = 'shoes/chanel/'
		...
		items = ImageItem.where("source = ? AND is_downloaded = ? AND item_type = ? AND manufacturer = ?", "chanel", false, "shoes", "Chanel")
