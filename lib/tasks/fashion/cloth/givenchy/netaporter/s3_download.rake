...
namespace :cloth_givenchy_netaporter do

   desc "Download Givenchy cloth data to AWS S3"
	task s3_download: :environment do

		main_bucket = ENV['AWS_BUCKET']
		main_folder = 'cloth/givenchy/'
		...
		items = ImageItem.where("source = ? AND is_downloaded = ? AND item_type = ? AND manufacturer = ?", "net-a-porter", false, "cloth", "Givenchy")
