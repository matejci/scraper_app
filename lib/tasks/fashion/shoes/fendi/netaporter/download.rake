...
namespace :shoes_fendi_netaporter_old do

	desc "Download Fendi shoes data on local disk"
	task download: :environment do

		main_path = "#{Rails.root}/downloaded_data/fashion/shoes/fendi/"

		items = Item.where("source = ? AND is_downloaded = ? AND item_type = ? AND manufacturer = ?", "net-a-porter", false, "shoes", "Fendi")
	...
	desc "Download Fendi shoes data to AWS S3"
	task s3_download: :environment do

		main_bucket = 'fashion-shoes-fendi'
