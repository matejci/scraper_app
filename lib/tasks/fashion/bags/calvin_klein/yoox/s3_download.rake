...
namespace :bags_ck_yoox do

	desc "Download Calvin Klein bags data to AWS S3"
	task s3_download: :environment do

		main_bucket = ENV['AWS_TEST_BUCKET']
		main_folder = 'bags/calvinklein/'
		...
		items = ImageItem.where("source = ? AND is_downloaded = ? AND item_type = ? AND manufacturer = ?", "yoox", false, "bags", "Calvin Klein")
