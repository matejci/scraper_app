...
namespace :cloth_dg_netaporter_old do

	desc "Download DolceGabbana cloth data on local disk"
	task download: :environment do

		main_path = "#{Rails.root}/downloaded_data/fashion/cloth/dolcegabbana/"

		items = Item.where("source = ? AND is_downloaded = ? AND item_type = ? AND manufacturer = ?", "net-a-porter", false, "cloth", "DolceGabbana")
	...
	desc "Download DolceGabbana cloth data to AWS S3"
	task s3_download: :environment do

		main_bucket = 'fashion-cloth-dolcegabbana'
