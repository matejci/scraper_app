...
namespace :bags_armani_yoox do

	desc "Download Armani bags data to AWS S3"
	task s3_download: :environment do

		main_bucket = ENV['AWS_BUCKET']
		main_folder = 'bags/armani/'
		...
		items = ImageItem.where("source = ? AND is_downloaded = ? AND item_type = ? AND manufacturer = ?", "yoox", false, "bags", "Armani")
