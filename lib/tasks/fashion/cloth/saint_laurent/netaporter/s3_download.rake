...
namespace :cloth_sl_netaporter do

   desc "Download Saint Laurent cloth data to AWS S3"
	task s3_download: :environment do

		main_bucket = ENV['AWS_BUCKET']
		main_folder = 'cloth/saintlaurent/'
		...
		items = ImageItem.where("source = ? AND is_downloaded = ? AND item_type = ? AND manufacturer = ?", "net-a-porter", false, "cloth", "SaintLaurent")
