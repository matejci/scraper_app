...
namespace :cloth_lp_yoox do

	desc "Download LaPerla cloth data to AWS S3"
	task s3_download: :environment do

		main_bucket = 'yewnofashion'
		main_folder = 'cloth/laperla/'
		...
		items = Item.where("source = ? AND is_downloaded = ? AND item_type = ? AND manufacturer = ?", "yoox", false, "cloth", "LaPerla")
