...
namespace :bags_moncler_moncler do
  ...
  desc "Download Moncler bags data to AWS S3"
	task s3_download: :environment do

		main_bucket = ENV['AWS_TEST_BUCKET']
		main_folder = 'bags/moncler/'
		...
		items = ImageItem.where("source = ? AND is_downloaded = ? AND item_type = ? AND manufacturer = ?", "moncler", false, "bags", "Moncler")
