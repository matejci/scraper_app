...
namespace :cloth_gucci_yoox do
	...
	task s3_download: :environment do
		...
		items = Item.where("source = ? AND is_downloaded = ? AND item_type = ? AND manufacturer = ?", "yoox", false, "cloth", "Gucci")
		...
		items.each do |item|
			...
			urls_arr.each do |url|
				...
				rescue URI::InvalidURIError => iuri
					logger.fatal("#{Time.now} :Error: #{iuri} -> ITEM_ID: #{item.id}, URL: #{url}")
