...
namespace :cloth_lp_lp_old do
	...
	task download: :environment do
		...
		items = Item.where("source = ? AND is_downloaded = ? AND item_type = ? AND manufacturer = ?", "laperla", false, "cloth", "LaPerla")
	...
	desc "Download La Perla cloth data to AWS S3"
	task s3_download: :environment do

		main_bucket = 'fashion-cloth-laperla'
