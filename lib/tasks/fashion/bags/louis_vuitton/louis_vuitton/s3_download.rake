...
namespace :bags_lv_lv do
  ...
  desc "Download Louis Vuitton bags data to AWS S3"
	task s3_download: :environment do

		main_bucket = ENV['AWS_TEST_BUCKET']
		main_folder = 'bags/louisvuitton/'
		...
		items = ImageItem.where("source = ? AND is_downloaded = ? AND item_type = ? AND manufacturer = ?", "louis_vuitton", false, "bags", "Louis Vuitton")
