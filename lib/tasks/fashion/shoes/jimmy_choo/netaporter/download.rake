...
namespace :shoes_jimmy_netaporter_old do

	desc "Download Jimmy Choo shoes data on local disk"
	task download: :environment do

		main_path = "#{Rails.root}/downloaded_data/fashion/shoes/jimmy_choo/"

		items = Item.where("source = ? AND is_downloaded = ? AND item_type = ? AND manufacturer = ?", "net-a-porter", false, "shoes", "Jimmy Choo")
	...
	desc "Download Jimmy Choo shoes data to AWS S3"
	task s3_download: :environment do

		main_bucket = 'fashion-shoes-jchoo'
