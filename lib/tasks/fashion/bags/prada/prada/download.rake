...
namespace :bags_prada_prada_old do

	desc "Download Prada bags data on local disk"
	task download: :environment do

		main_path = "#{Rails.root}/downloaded_data/fashion/bags/prada/"

		items = Item.where("source = ? AND is_downloaded = ? AND item_type = ? AND manufacturer = ?", "prada", false, "bags", "Prada")
	...
	desc "Download Prada bags data to AWS S3"
	task s3_download: :environment do

		main_bucket = 'fashion-bags-prada'
