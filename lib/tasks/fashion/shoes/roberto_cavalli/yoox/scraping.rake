...
namespace :shoes_rc_yoox do

	desc "Scrap Roberto Cavalli shoes data"
	task scrap: :environment do
		file_content_array = []

		File.foreach("#{Rails.root}/lib/tasks/fashion/shoes/roberto_cavalli/yoox/shoes.csv") do |line|
		...
		file_content_array.each do |link|
			begin
				...
				image = doc.css('#itemImage').css('img').attr('src').value
