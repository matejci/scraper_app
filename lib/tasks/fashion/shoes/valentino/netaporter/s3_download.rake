...
namespace :shoes_valentino_netaporter do

   desc "Download Valentino shoes data to AWS S3"
	task s3_download: :environment do

		main_bucket = ENV['AWS_TEST_BUCKET']
		main_folder = 'shoes/valentino/'
		...
		items = ImageItem.where("source = ? AND is_downloaded = ? AND item_type = ? AND manufacturer = ?", "net-a-porter", false, "shoes", "Valentino")
