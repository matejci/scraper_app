...
namespace :shoes_tomford_netaporter_old do

	desc "Download TomFord shoes data on local disk"
	task download: :environment do

		main_path = "#{Rails.root}/downloaded_data/fashion/shoes/tomford/"

		items = Item.where("source = ? AND is_downloaded = ? AND item_type = ? AND manufacturer = ?", "net-a-porter", false, "shoes", "TomFord")
	...
	desc "Download TomFord shoes data to AWS S3"
	task s3_download: :environment do

		main_bucket = 'fashion-shoes-tomford'
