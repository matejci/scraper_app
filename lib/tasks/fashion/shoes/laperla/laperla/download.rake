...
namespace :shoes_lp_lp_old do

	desc "Download La Perla shoes data on local disk"
	task download: :environment do

		main_path = "#{Rails.root}/downloaded_data/fashion/shoes/laperla/"

		items = Item.where("source = ? AND is_downloaded = ? AND item_type = ? AND manufacturer = ?", "laperla", false, "shoes", "LaPerla")
	...
	desc "Download La Perla shoes data to AWS S3"
	task s3_download: :environment do

		main_bucket = 'fashion-shoes-laperla'
