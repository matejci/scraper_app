...
namespace :shoes_ck_mytheresa do

  desc "Download Calvin Klein shoes data to AWS S3"
	task s3_download: :environment do

		main_bucket = ENV['AWS_BUCKET']
		main_folder = 'shoes/calvinklein/'
		...
		items = ImageItem.where("source = ? AND is_downloaded = ? AND item_type = ? AND manufacturer = ?", "mytheresa", false, "shoes", "Calvin Klein")
