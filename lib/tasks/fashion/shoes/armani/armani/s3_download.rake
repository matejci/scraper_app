...
namespace :shoes_armani_armani do

  desc "Download Armani shoes data to AWS S3"
	task s3_download: :environment do

		main_bucket = ENV['AWS_BUCKET']
		main_folder = 'shoes/armani/'
		...
		items = ImageItem.where("source = ? AND is_downloaded = ? AND item_type = ? AND manufacturer = ?", "armani", false, "shoes", "Armani")
